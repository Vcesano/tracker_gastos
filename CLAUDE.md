...
| 13 | 🔴 | `fechaISO_` (`migracion.js`) | **No salió de la revisión estática: lo encontraron los tests de 4b al correr contra Sheets de verdad.** Formateaba con `America/Argentina/Tucuman` hardcodeada, pero Sheets guarda las fechas como serial y las interpreta en la timezone **de la spreadsheet**. Contra una spreadsheet en otra zona, TODAS las fechas vuelven corridas un día — y de paso el guard de duplicados de Pago Bulk deja de funcionar **en silencio**, porque compara `compra\|fecha` contra la fecha ya corrida. | ✅ 4b (deploy @26) |

Al completar una iteración, marcarla acá y anotar lo decidido en "Registro de decisiones".

## Registro de decisiones

- 2026-07-25 (It 4b, fechas y timezone): **las fechas se formatean con la timezone de la SPREADSHEET, nunca con una hardcodeada.** Sheets guarda una celda de fecha como número de serie y lo interpreta en la tz de la spreadsheet; formatear con otra corre el día. Como `fechaISO_` se llama una vez por fila, la tz se memoiza por ejecución (`TZ_MEMO_` en `db.js`, reseteada cuando `abrirSS_` reabre). Corolario para el futuro: **cualquier spreadsheet nueva del proyecto (sandbox incluida) debe tener la tz del proyecto**, y todo dato que viaje por Sheets y vuelva necesita un test de round-trip, no solo de escritura.
