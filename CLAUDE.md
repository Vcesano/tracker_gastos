...
- [ ] **It 0 — Andamiaje + copia de datos**: Valentin provee la copia de la spreadsheet (desconectada de AppSheet, editable). Repo GitHub, clasp conectado, `SHEET_ID` → copia. En la copia: renombrar las pestañas existentes con prefijo `legacy_` y crear las 4 pestañas nuevas con headers. `doGet` mínimo deployado privado, probado desde el celu.
