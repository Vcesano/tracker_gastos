...
/tests/                    # It 4: NO se pushea con clasp (vive fuera de /src)
  ...
  checklist-manual.md      # It 4g: 48 casos manuales (celu + PC) con resultado esperado
...
- [ ] **It 4 — QA, hardening y accesibilidad** (antes de Power BI): la app está funcionalmente completa y linda, pero nunca fue testeada sistemáticamente ni tiene red de seguridad ante regresiones. It 4 la audita a fondo (correctitud, performance medida, UX/UI, accesibilidad, robustez de datos), arregla lo que salga y deja una **suite de tests re-ejecutable** para que It 5/It 6 no rompan nada en silencio. Regla de oro de la iteración: **ningún test toca la copia de trabajo** — los tests de integración corren contra una spreadsheet sandbox aparte.
  ...
  - [x] **4c — tests de la lógica pura del cliente** (deploy @28, 2026-07-25): **24 tests** en `tests/client-tests.js`, se pegan en la consola del navegador (PC) sobre la app real e imprimen PASS/FAIL. Sin build step, sin duplicar código: `app-js.html` expone en `window.__TEST__` las funciones **que ya existen**.
    - Para que hubiera algo puro que testear se extrajeron dos funciones que estaban inline: **`filtrarPorTexto(lista, q, campos)`** (reemplaza las 3 copias que había en Historial, Compras y Pendientes) y **`agruparPorDia(gastos)`** (sacada del medio de `renderHistorial`). Menos duplicación y, de paso, testeable.
    - Cubre: formato de monto es-AR (incluido el **round-trip `montoAInput` → `parseMonto`**, que es lo que garantiza que editar un gasto no le cambie el importe), fechas dd/mm/yyyy y día de la semana sin corrimiento por UTC, coherencia `hoyISO`/`fechaMenosDias` (atajos Hoy/Ayer), `fmtMoneda`, helpers de lista, búsqueda por texto y agrupación por día **con totales por moneda separados**.
    - `/tests/` vive fuera de `/src`: clasp no lo sube (verificado).
  - [x] **4d — performance medida** (2026-07-25, deploy @29)
    - **Números del server** (sandbox, 3 reps en frío, ms p50):

      | endpoint | 350 gastos | 5.003 gastos |
      |---|---|---|
      | `getCatalogos` | 437 | 318 |
      | `getMaestros` | 314 | 312 |
      | **`listarGastos` (sin filtros)** | **967** | **4159** |
      | `listarGastos` (rango de 1 mes) | 904 | 4325 |
      | `listarGastos` (solo cuotas) | 857 | 4351 |
      | `listarCompras` (todas) | 770 | 1091 |
      | `crearGasto` (escritura) | 1072 | 1730 |

    - **Hallazgo contraintuitivo: filtrar en el server no ahorra nada.** `listarGastos` con un rango de 1 mes tarda lo mismo (o más) que sin filtros, porque el costo está en leer y mapear la pestaña **entera** y el filtro se aplica después, sobre los objetos ya construidos. Corolario: **no agregar filtros server-side esperando velocidad** (solo reducen payload), y si algún día hay que acelerar, la única palanca real es **leer menos filas de la Sheet**, no filtrar mejor.
    - Modelo: **`listarGastos ≈ 727 ms fijos + 0,69 ms por gasto`**. Los ~727 ms fijos son abrir la spreadsheet + leer las otras 3 tablas; hay un **piso de ~300 ms por request** (`getMaestros` con datos mínimos ya cuesta 312 ms) que no baja sin cambiar de arquitectura. `getCatalogos`/`getMaestros` no dependen del volumen de `Gastos`, como se esperaba.
    - **Números del cliente** (`__PERF__`, app real, 361 gastos):

      | operación | p50 |
      |---|---|
      | `renderHistorial` (DOM, 361 gastos) | **22 ms** |
      | `renderHistorial` (DOM, 57-62 gastos) | 5-6 ms |
      | `listarGastos` (ida y vuelta) | **3598 ms** (server: 967) |
      | `listarCompras` (ida y vuelta) | 3305 ms (server: 770) |
      | `getMaestros` (ida y vuelta) | 2365 ms (server: 314) |

    - **Hallazgo que reordena las prioridades: el cuello de botella es el transporte de `google.script.run`, no nuestro código.** `getMaestros` tarda 314 ms de server y **2365 ms** punta a punta: ~2 s son puro overhead de Apps Script. En `listarGastos`, el trabajo propio es apenas el **27%** del tiempo total. Corolario: **optimizar el server tiene poca palanca** — aunque `listarGastos` fuera instantáneo, el Historial seguiría tardando ~2,6 s en frío. Lo único que mueve la aguja de verdad es **hacer menos viajes**, que es exactamente lo que hizo It 3b (inyección de catálogos en el HTML + cache stale-while-revalidate) y por eso fue lo que más se notó.
    - **El render del DOM es un no-problema: 22 ms para 361 gastos** (~0,06 ms/gasto). Cierra la otra mitad del hallazgo #5: **no hace falta render incremental ni scroll virtual**.
    - **Decisión: NO se pagina ahora.** A 350 gastos es ~1 s de server (sobre ~3,6 s totales, o sea que paginar atacaría el 27% del problema) y el cache client-side lo tapa casi siempre. **Umbral para revisar: `Gastos` > ~2.000 filas.** Llegado ese punto la solución NO es más filtros ni menos payload: es **hacer menos viajes** y, recién después, leer menos filas (que choca con los gastos de fecha retroactiva, porque el orden físico de la Sheet no es el orden por fecha).
    - Caveat de la medición del cliente: se navegó rápido, así que algunas llamadas pudieron encolarse e inflarse entre sí. `getMaestros` con n=1 a 2365 ms es la evidencia más limpia del piso.
    - **Acción derivada (deploy @30): `precargarVistas()`** — al arrancar se precalientan Historial (como antes), **Crédito-Compras y Maestros**. Es la única optimización que los datos justifican: no acelera nada, **saca el viaje del camino crítico**. El primer clic en cada pestaña pasa de ~3 s a instantáneo. El Historial sale primero y solo (destino más probable); las otras dos, 600 ms después, para no competir con el primer pintado. Reusa los `cargar*()` de siempre y se apoya en el guard de pedidos en vuelo de 4a para no duplicar llamadas.
    - Instrumentación (deploy @29): `medirPerf({repeticiones, filas})` en `tests.js` — dos escenarios sobre la sandbox, min/p50/máx. **Cada repetición arranca en frío** (se tiran `SS_MEMO_` y `TABLA_MEMO_`): la memoización de It 3b vive dentro de una ejecución, así que medir con el memo caliente daría números lindos y mentirosos. En el cliente, `window.__PERF__.on()` / `.tabla()` mide con `performance.now()` la ida y vuelta de cada `google.script.run` **y el render del DOM** de `renderHistorial` por separado — la distinción que define si un problema del Historial se arregla en el server o en el render. Apagado por defecto: con el flag en false, medir es un `if`.: `medirPerf({repeticiones, filas})` en `tests.js` cronometra 8 endpoints en dos escenarios sobre la sandbox — realista (350 gastos, ≈ el volumen de hoy) y **carga (5.000 gastos)** — y reporta min/p50/máx. **Cada repetición arranca en frío** (se tiran `SS_MEMO_` y `TABLA_MEMO_`): la memoización de It 3b vive dentro de una ejecución, así que medir con el memo caliente daría números lindos y mentirosos. En el cliente, `window.__PERF__.on()` / `.tabla()` mide con `performance.now()` la ida y vuelta de cada `google.script.run` **y el render del DOM** de `renderHistorial` por separado — que es la distinción que define si el problema del Historial se arregla con paginación server o con render incremental. Apagado por defecto: con el flag en false, medir es un `if`.
  - [x] **4e — accesibilidad y pulido de UI** (deploy @31, 2026-07-25):
    - **Foco visible**: regla global `:focus-visible` (anillo de 3px en `--accent`). Se usa `:focus-visible` y no `:focus` a propósito: el navegador no lo dispara al tocar ni al clickear, así que el anillo aparece navegando con Tab y no ensucia el uso en el celu. Los campos lo llevan pegado (`outline-offset: 0`) porque ya resaltan su borde. Cierra el hallazgo **#11**.
    - **Patrón ARIA de tablist completo**: `role="tablist"` en las pestañas y en los 2 grupos de subtabs, `role="tab"` + `aria-selected` + `aria-controls`, `role="tabpanel"` + `aria-labelledby` en las vistas, **roving tabindex** (solo el tab activo es tabulable — si no, Tab tendría que pasar por las 5 pestañas para llegar al contenido) y **navegación con flechas ← → / Inicio / Fin**. Centralizado en `marcarTab()` + `activarTeclasTablist()`.
    - **Contraste verificado y corregido** (AA = 4.5:1 para texto normal). `--muted` ya pasaba (5.68 sobre `--card`, 6.27 sobre `--bg`) — se verificó, no se tocó. **`--err` no pasaba y se partió en dos tokens**, porque el rojo se usaba de dos formas opuestas y una sola tinta no puede servir a ambas: `--err` (#d13c33) es el **fondo** del botón destructivo con texto blanco (era 3.70 → **4.77**), y `--err-text` (#f2776e) es el **texto** rojo sobre tarjeta (era 4.00 → **5.40**). Aclarar el rojo arreglaba el texto y empeoraba el botón, y viceversa. Final: text 12.33 · muted 5.68 · err-text 5.40 · warn 6.88 · ok 7.42 · blanco/btn-danger 4.77 · on-accent 8.93 — **todo AA**.
    - **Targets táctiles**: `.icon-btn` (Editar/Borrar del Historial, los más tocados con el pulgar) de ~24px a **36px**, y `.link` de ~21px a **32px**. Los 44px de AAA obligarían a rediseñar la fila; 36px agranda la fila ~12px sin romper nada.
    - **Placeholder de selects obligatorios**: `r-tarjeta` / `r-medio` reusaban `fillFiltro`, cuya primera opción es un "Todos" **elegible** — permitía volver a "sin elegir" en un campo requerido. `fillFiltro` gana un flag `phDeshabilitado`. Segunda mitad del hallazgo **#11**.
    - Ya estaban hechos en 4a: zoom en mobile (`maximum-scale` fuera), focus trap + devolución de foco + scroll bloqueado del modal, y `role="status"` en el toast.
  - [x] **4f — robustez de datos** (deploy @32, 2026-07-26): **18 tests nuevos** (12 unitarios + 6 de integración), total **70**. Cierra los hallazgos **4, 7 y 8**.
    - **#4 — atomicidad real de `confirmarResumen`**: el leer→validar→escribir corre entero adentro de un solo lock. `db.js` gana `tomarLock_()`, `conLock_(fn)` (toma el lock **y tira el cache de lectura**, para que las `leerTabla_` de adentro vean el estado del momento en que se ganó) e `insertarFilasSinLock_`. **No se puede anidar el lock**: dos `tryLock` en la misma ejecución se traban solos, así que adentro de `conLock_` hay que escribir con las variantes `*SinLock_`. El cuerpo se extrajo a `confirmarResumenEnLock_`. Efecto colateral en los tests: `conFixtures_` ahora también neutraliza `conLock_` (si no, su `invalidarTabla_()` borraría los fixtures en memoria).
    - **#7 — ids únicos**: `nuevoIdUnico_(tabla, reservados)` chequea contra los ids ya escritos **y** contra los reservados en el mismo batch (que todavía no están en la Sheet). Lo usan las 4 altas y `confirmarResumen`. Barato: la tabla ya está en el memo de la ejecución.
    - **#8 — límites de longitud server-side**: `LIMITES_TEXTO` + `textoLimitado_()` en `logic.js`, aplicados en los 4 validadores y en cada ítem de `confirmarResumen`. Los números replican los `maxlength` del form (descripción/nota 140, categoría/subcategoría 60, entidad 80) para que no exista un texto que el cliente acepte y el server rechace.
    - **Defensivo ante una Sheet editada a mano**: `esActivo_` acepta las formas razonables (`true`/`1`/`TRUE`/`VERDADERO`/`SI`/`X`) y trata **todo lo demás como inactivo** — el default seguro es ocultar un maestro dudoso, no ofrecerlo para cargar. `numeroSeguro_()` (sobre `numero_`) garantiza un número finito en todos los lectores: un monto tipeado `1.234,56` se lee bien y uno con basura cae a 0 en vez de propagar `NaN`. Las **FKs rotas ya no se disfrazan de id opaco**: `etiquetaRota_()` las muestra como `(categoría inexistente: zzz99999)` en la pantalla donde están.
    - **`diagnosticarDatos()` + `reporteDiagnostico()`** (nuevos, read-only, no cableados a ninguna pantalla): auditan las 4 pestañas e informan ids vacíos/duplicados, FKs huérfanas, fechas fuera de formato, montos no numéricos, monedas fuera de ARS/USD, `n_cuotas` inválidos y compras con más pagos vinculados que cuotas. Se corren desde el editor sobre la copia de trabajo.
    - Ojo con un detalle que casi se testea mal: **`numero_('60.000')` da 60**, no 60.000 — sin coma decimal el punto es ambiguo. El test usa `'60.000,00'`, que sí es inequívocamente es-AR.
  - [x] **4g — checklist manual guiada** (2026-07-26): `tests/checklist-manual.md` — **48 casos** en 6 bloques (A preparación, B gestos táctiles 📱, C teclado y navegación 💻, D bordes de datos, E red/sesión/estados raros, F tamaños y limpieza), cada uno con **cómo probarlo y qué tiene que pasar**. Cubre a propósito lo que ningún test automático puede ver: pinch-zoom, teclado numérico, calendario nativo, focus trap con Tab real, rotación, red lenta y sin red, doble submit, sesión vencida. Empieza y termina con `reporteDiagnostico()` (A1 y F4) para probar que la corrida **no dejó datos rotos**, y el paso F3 obliga a limpiar lo que se creó. Vive en `/tests/` → no lo sube clasp. **Lo corre Valentin**: es el entregable con el que se cierra It 4.
...
- [ ] **It 6 — Reporte + resguardo**: páginas restantes del reporte (mensual por categoría/medio, tendencias MoM, tablero de cuotas y compromisos futuros, corte por moneda), backup semanal automático de la copia de trabajo (trigger horario de GAS), re-corrida de migración para el cutover.
  - **Checklist del cutover** (no olvidar, viene del hallazgo #14): (1) backup de la copia de trabajo **antes** de re-correr `migrar()`; (2) re-copiar las pestañas frescas de la original a `legacy_*`; (3) correr `migrar()` y **leer el bloque "Backfill de categoría faltante"** del reporte — cuántas heredaron de la compra, cuántas se resolvieron por texto y, sobre todo, **la lista de las que quedaron sin resolver** (esas se deciden a mano); (4) correr `reporteDiagnostico()` y exigir `✓ Sin problemas`. **Ojo con el orden**: los gastos cargados desde la app **no están** en `legacy_*`, así que re-correr la migración los borraría — hay que definir cómo se preservan antes de tocar nada.
...
| 5 | 🟡 | `listarGastos` + `renderHistorial` | Sin límite ni paginación: siempre trae y **renderiza los 345 gastos** (y crece para siempre). Además la búsqueda por texto no tiene debounce → re-render completo del DOM por tecla. | 🟠 debounce ✅ 4a; paginación **descartada por ahora con datos** (4d): a 350 gastos son ~1 s que el cache tapa. Umbral para revisar: **>2.000 filas** |
...
| 14 | 🟡 | `migrar()` (`migracion.js`, validación de FKs) | **No salió de la revisión estática ni de los tests: lo encontró `reporteDiagnostico()` al correr sobre los datos reales** (It 4f, 2026-07-26). **67 de 361 gastos (19%) tienen `categoria_id` vacío.** La validación de la migración reportó "0 fks huérfanas" porque la condición era `if (legacyCat && !catMap[legacyCat])`: **el guard `legacyCat &&` hacía invisible el caso vacío**. No es corrupción — son filas de AppSheet que nunca tuvieron el vínculo. | ✅ Corregido en la migración (ver decisión del 2026-07-26). **Los datos actuales NO se tocan**: se rehacen en el cutover |
...
- 2026-07-26 (It 4f, los 67 gastos sin categoría — hallazgo #14): **no se arreglan los datos actuales, se arregla la migración.** Decisión de Valentin: al cutover se rehace la migración desde la spreadsheet original, así que corregir a mano las 67 filas de hoy sería trabajo tirado (y se volvería a romper igual). Lo que sí se hizo es cerrar el agujero en `migrar()` para que el cutover salga bien:
  - **Cascada de backfill de `categoria_id`, en orden y sin inventar nada**: (1) la FK legacy si mapea — camino normal; (2) **si el gasto es cuota, hereda la categoría de su compra** — no es una heurística, es la regla del modelo (`ComprasCredito.categoria_id` es la "categoría default que heredan sus cuotas"); (3) el texto `Tipo_de_Gasto` + `Categoria` del propio `legacy_Gastos` (columnas que hasta ahora se descartaban por duplicar la FK), **solo si resuelve a UNA categoría** — con varias subcategorías candidatas se prefiere la fila sin subcategoría, y si sigue habiendo empate no se adivina; (4) lo que no entra en ninguna queda vacío **y se lista con id, fecha y descripción en el reporte** para decidirlo a mano.
  - **El reporte de migración ahora separa tres cosas que antes se mezclaban en una sola línea**: FKs *huérfanas* (apuntan a algo inexistente), FKs *vacías* (falta el dato) y el desglose del backfill. Esa distinción es exactamente la que faltaba: la corrida de It 1 dijo "0 huérfanas" y era cierto — las 67 estaban vacías, que es otro problema.
  - **La lección de método, que vale más que el fix**: una validación con forma `if (valor && esInvalido(valor))` **no valida el caso vacío, lo esconde**. Y esto no lo encontró ni la revisión estática ni los 70 tests: lo encontró correr un diagnóstico contra **datos reales**. Los fixtures tienen los casos que uno se imagina; la Sheet tiene los que pasaron de verdad.
- 2026-07-26 (It 4f, dónde va cada defensa): la app es el **único escritor sano**, pero la Sheet está a un clic y Valentin la abre para mirar. Se separó en tres capas y conviene no mezclarlas: (a) **validar** lo que entra (límites de longitud, montos, monedas, FKs activas) — rechaza y explica; (b) **coercionar** lo que sale (`numeroSeguro_`, `esActivo_`, `etiquetaRota_`) — nunca romper una pantalla por una celda mal tipeada, y **mostrar el problema en vez de esconderlo** detrás de un id opaco; (c) **diagnosticar** a pedido (`reporteDiagnostico()`) — lo que ninguna de las dos anteriores puede arreglar sola. Regla derivada: un dato dudoso se trata como **inactivo/0/etiquetado**, nunca se adivina.
- 2026-07-26 (It 4f, locks): `LockService` **no es reentrante en la práctica** — tomar un segundo `tryLock` dentro de una ejecución que ya tiene el lock se traba hasta el timeout. Por eso toda escritura tiene ahora dos formas: la pública (toma el lock) y la `*SinLock_` (asume que ya lo tenés). Si en el futuro hace falta otra operación "leer→decidir→escribir" atómica, el patrón es `conLock_(function(){ ... insertarFilasSinLock_(...) })`, nunca llamar a `insertarFilas_` adentro.

- 2026-07-25 (It 4d, dónde está el tiempo): **el cuello de botella de la app es el transporte de `google.script.run` (~2-2,6 s por llamada), no el código del server ni el render.** Medido: `getMaestros` = 314 ms de server contra 2365 ms punta a punta; `renderHistorial` = 22 ms para 361 gastos. Consecuencia práctica para todo lo que venga: **la única optimización con palanca real es hacer menos viajes al server** (inyectar datos en el HTML inicial, cachear en el cliente, agrupar llamadas). Filtrar mejor, paginar o reducir payload atacan una fracción chica del tiempo. Esto también explica retroactivamente por qué It 3b se sintió tanto.
- 2026-07-25 (It 4b, verificación en producción): la copia de trabajo guarda las fechas como `Date` **y siempre se mostraron bien**, porque su timezone (`America/Buenos_Aires`, UTC-3) coincide con la del script (Tucumán): el `Date` vuelve como medianoche UTC-3 y tanto el formateo viejo (tz del proyecto) como el nuevo (UTC) dan la misma fecha. O sea: **el bug #13 nunca afectó datos ni pantallas reales**; solo se manifestaba contra una spreadsheet en otra zona, como la sandbox (que nació en `America/Los_Angeles`). El fix de UTC quedó verificado contra **las dos** zonas; el código viejo solo funcionaba en UTC-3.
