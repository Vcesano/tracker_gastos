...
- [~] **It 1 — Migración + maestros + gasto diario** (EN CURSO):
  ...
  - [ ] **Falta la parte de app** (retomar acá en sesión nueva; el modelo operacional ya está poblado). Plan en 3 slices chicos y deployables:
    - **1a — capa de datos + carga rápida**: crear `db.js` (capa CRUD de tablas operacionales: `abrirSS_`, `leerTabla_` → filas como objetos por header, insertar/actualizar/borrar en batch con `LockService`; **reusar los globals ya existentes en `migracion.js`**: `getSheetId_`, `nuevoId_`, `indexar_` — no redeclararlos, viven en el mismo scope global de Apps Script). `api.js` con endpoints `{ok, data?|error?}`. Pantalla estrella del día a día: form de gasto con selects en cascada Tipo→Categoria→Subcategoria (solo `activo=TRUE`), medio de pago, monto (>0, teclado numérico), fecha default hoy, moneda default `ARS` editable, descripción opcional. Inserta 1 fila en `Gastos` con `nuevoId_()`. Prueba: cargar desde el celu y ver la fila en la Sheet.
    - **1b — historial**: lista de `Gastos` con filtros por mes/categoría/medio; editar y borrar (borrado **físico** solo en `Gastos`, con confirmación). Dereferenciar categoria_id/medio_pago_id a etiquetas.
    - **1c — ABM maestros**: alta/edición/**desactivar** (soft delete `activo=FALSE`, nunca borrar si hay referencias) de `Categorias` y `MediosPago`. `logic.js` para validaciones/cascadas.
  - Convenciones a respetar: mobile-first, vanilla JS/CSS sin frameworks/CDNs, `db.js` lee tabla completa por header y escribe en batch (nunca fila por fila en loop), toda escritura con lock. El andamiaje cliente actual (`index/styles/app-js.html`) es el smoke test de It 0 — se expande, hay un `ping()` en `Code.js` para remover.
